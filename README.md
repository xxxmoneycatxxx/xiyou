...
- PHP 8.2（需 `pdo_mysql` `curl` `bcmath` `mbstring` `opcache` `redis` 扩展）
- MySQL 8.0（默认 InnoDB）
- Redis 7（INI 缓存后端）
...
- **分区**（`fqxy/`）：每个分区是独立的游戏世界，拥有完整的 `xyy` 库（74 张表）和独立的 Redis 缓存。通过 `config.php` 的 `urls` 数组可配置无限个分区，共享同一套家园认证。
...
| Redis Hash | `ini:{wjid}:{file}:{section}` | 玩家运行时状态（地图坐标、当前页面、验证信息） | 内存级读写，O(1) |
| INI 文件 | `fqxy/acher/{wjid}/` | 回退缓存（Redis 不可用时自动降级） | 与 Redis 自动同步 |

#### 缓存数据结构

每个玩家在 Redis 中以 Hash 存储多 section 数据，对应原 `.ini` 文件：
- `ini:{wjid}:user:地图坐标` → `{x: 1, y: 96}`
- `ini:{wjid}:zt:玩家信息` → 从 `all_zt` 表缓存的玩家属性
- `ini:{wjid}:sjyz:时间验证` → 微秒时间戳，防刷新
- 各地图区域 cache：如 `ini:{wjid}:xsc0x1`（新手村 0-1 坐标的状态）

#### `iniFile` 类 (`fqxy/class/iniclass.php`)

核心缓存引擎（v2.0）。底层从文件 INI 升级为 Redis Hash，**API 完全兼容**。读写策略：Redis 优先 — `save()` 先写 Redis，不可用时自动回退文件 I/O；`__construct()` 优先从 Redis 加载，缺失时读文件并自动写入 Redis（首次迁移）。提供 `getItem/addItem/updItem/delItem` 等 7 个 CRUD 方法，350+ 调用方零改动。

> 设计权衡：Redis Hash 消除了每次属性变更的全量文件重写（单次战斗请求可触发 20+ 次 `updItem` 磁盘 I/O），读写延迟从 ms 降至 μs 级。同时消除了文件并发写覆盖的竞态风险。
...
├── docker/                      # Docker 部署
│   ├── Dockerfile               #   镜像定义（php:8.2-apache + redis 扩展）
│   ├── docker-compose.yml       #   服务编排（MySQL 8.0 + Redis 7 + Apache）
...
│   ├── ini/                     # 公共 INI 缓存
│   ├── ache/{wjid}/             # 玩家 INI 缓存（运行时生成，Redis 可替代）
│   ├── acher/                   # 地图 INI 缓存（坐标、超链接白名单）
...
| **缓存** | Redis Hash + INI 文件回退，读写 O(1)，API 兼容 350+ 调用方 |
...
2. **INI 缓存已升级为 Redis**：`iniFile` 类（v2.0）底层为 Redis Hash，文件 I/O 仅作回退。API 完全兼容，无需修改调用方。
...
- 修复多项影响正常游玩的 Bug
- 删除原家园冗余页面
- **新增 Docker 部署支持**（一键部署，含 MySQL 8.0 + php:8.2-apache + Redis 7）
...
- **INI 文件缓存 → Redis**：底层从文件 I/O 升级为 Redis Hash，读写延迟 ms→μs，消除并发竞态，350+ 调用方零改动（#8）
