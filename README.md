...
| INI 文件 | `fqxy/ache/{wjid}/` | 玩家运行时状态（地图坐标、当前页面、验证信息） | 每次读写都即时落盘 |

#### INI 文件结构

每个玩家在 `fqxy/ache/` 下有独立目录，包含多个 `.ini` 文件：
...
2. **INI 缓存是单点依赖**：`iniFile` 类被几乎所有游戏页面引用，修改其行为前需要充分评估影响。
